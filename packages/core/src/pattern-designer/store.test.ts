...
describe('PatternDesignerStore', () => {
  ...
  // =========================================================================
  // Grid Resize Actions (Iteration 2.7)
  // =========================================================================
  ...
  describe('grid resize actions', () => {
    ...
    describe('addRow', () => {
      it('adds a row at the bottom by default', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        ...
        const { gridSize } = usePatternDesignerStore.getState().pattern;
        expect(gridSize.rows).toBe(4);
        expect(gridSize.cols).toBe(3);
      });

      it('adds a row at the top when position is "top"', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        // Place a block at (0, 0)
        store.addBlockInstance('block-1', { row: 0, col: 0 });

        const result = store.addRow('top');

        expect(result).toBe(true);
        const { gridSize, blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(gridSize.rows).toBe(4);
        // Block should now be at row 1 (shifted down)
        expect(blockInstances[0].position.row).toBe(1);
        expect(blockInstances[0].position.col).toBe(0);
      });

      it('shifts all blocks down when adding row at top', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 });
        store.addBlockInstance('block-2', { row: 1, col: 1 });
        store.addBlockInstance('block-3', { row: 2, col: 2 });

        store.addRow('top');

        const { blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(blockInstances[0].position.row).toBe(1);
        expect(blockInstances[1].position.row).toBe(2);
        expect(blockInstances[2].position.row).toBe(3);
      });

      it('returns false when at maximum row limit', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 25, cols: 3 }); // MAX_GRID_SIZE is 25
      ...
      it('sets isDirty to true after adding a row', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        expect(usePatternDesignerStore.getState().isDirty).toBe(false);
    ...
    describe('removeRow', () => {
      it('removes the bottom row by default', () => {
        const store = usePatternDesignerStore.getState();
        ...
        expect(result).toBe(true);
        expect(usePatternDesignerStore.getState().pattern.gridSize.rows).toBe(3);
      });

      it('removes blocks in the bottom row when removing from bottom', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 });
        store.addBlockInstance('block-2', { row: 2, col: 0 }); // This will be removed

        store.removeRow('bottom');

        const { blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(blockInstances).toHaveLength(1);
        expect(blockInstances[0].position.row).toBe(0);
      });

      it('removes the top row when position is "top"', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 4, cols: 3 });

        const result = store.removeRow('top');

        expect(result).toBe(true);
        expect(usePatternDesignerStore.getState().pattern.gridSize.rows).toBe(3);
      });

      it('removes blocks in row 0 and shifts remaining blocks up when removing from top', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 }); // This will be removed
        store.addBlockInstance('block-2', { row: 1, col: 1 });
        store.addBlockInstance('block-3', { row: 2, col: 2 });
        ...
        // Blocks should be shifted up
        expect(blockInstances[0].position.row).toBe(0); // was row 1
        expect(blockInstances[1].position.row).toBe(1); // was row 2
      });

      it('returns false when at minimum row limit', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 2, cols: 3 }); // MIN_GRID_SIZE is 2
      ...
      it('clears selection if selected block is removed', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        const instanceId = store.addBlockInstance('block-1', { row: 2, col: 0 });
        store.selectBlockInstance(instanceId);
    ...
    describe('addColumn', () => {
      it('adds a column at the right by default', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        ...
        expect(gridSize.cols).toBe(4);
      });

      it('adds a column at the left when position is "left"', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        // Place a block at (0, 0)
        store.addBlockInstance('block-1', { row: 0, col: 0 });

        const result = store.addColumn('left');

        expect(result).toBe(true);
        const { gridSize, blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(gridSize.cols).toBe(4);
        // Block should now be at col 1 (shifted right)
        expect(blockInstances[0].position.row).toBe(0);
        expect(blockInstances[0].position.col).toBe(1);
      });

      it('shifts all blocks right when adding column at left', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 });
        store.addBlockInstance('block-2', { row: 1, col: 1 });
        ...
        store.addColumn('left');

        const { blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(blockInstances[0].position.col).toBe(1);
        expect(blockInstances[1].position.col).toBe(2);
        expect(blockInstances[2].position.col).toBe(3);
      });

      it('returns false when at maximum column limit', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 25 }); // MAX_GRID_SIZE is 25
    ...
    describe('removeColumn', () => {
      it('removes the right column by default', () => {
        const store = usePatternDesignerStore.getState();
        ...
        expect(result).toBe(true);
        expect(usePatternDesignerStore.getState().pattern.gridSize.cols).toBe(3);
      });

      it('removes blocks in the last column when removing from right', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 });
        store.addBlockInstance('block-2', { row: 0, col: 2 }); // This will be removed

        store.removeColumn('right');
      ...
      it('removes the left column when position is "left"', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 4 });

        const result = store.removeColumn('left');

        expect(result).toBe(true);
        expect(usePatternDesignerStore.getState().pattern.gridSize.cols).toBe(3);
      });

      it('removes blocks in col 0 and shifts remaining blocks left when removing from left', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 }); // This will be removed
        store.addBlockInstance('block-2', { row: 0, col: 1 });
        store.addBlockInstance('block-3', { row: 0, col: 2 });

        store.removeColumn('left');

        const { blockInstances } = usePatternDesignerStore.getState().pattern;
        expect(blockInstances).toHaveLength(2);
        // Blocks should be shifted left
        expect(blockInstances[0].position.col).toBe(0); // was col 1
        expect(blockInstances[1].position.col).toBe(1); // was col 2
      });

      it('returns false when at minimum column limit', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 2 }); // MIN_GRID_SIZE is 2
    ...
    describe('hasBlocksInRow', () => {
      it('returns true if row contains blocks', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 1, col: 0 });

        expect(store.hasBlocksInRow(1)).toBe(true);
      });
      ...
      it('returns false if row is empty', () => {
        ...
        expect(store.hasBlocksInRow(2)).toBe(false);
      });
    });

    describe('hasBlocksInColumn', () => {
      it('returns true if column contains blocks', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 1 });

        expect(store.hasBlocksInColumn(1)).toBe(true);
      });

      it('returns false if column is empty', () => {
        const store = usePatternDesignerStore.getState();
        store.initPattern({ rows: 3, cols: 3 });
        store.addBlockInstance('block-1', { row: 0, col: 0 });

        expect(store.hasBlocksInColumn(2)).toBe(false);
      });
    });
  });
